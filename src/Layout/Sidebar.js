      ...
      <div className="flex-colo gap-2 mt-12">
        {MenuDatas.map((item, index) => (
          <Link
            ...
            key={index}
            className={`
            ${currentPath(item.path) === item.path ? 'ghjjjs-text' : ''}
             gap-4 transitions group items-center w-full p-4 rounded-lg hover:bg-text flex_admin`}
          >
            <item.icon
              className={`text-xl jkkky_color 
            `}
            />
            <p
