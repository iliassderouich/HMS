...
export function Transactiontable({ data, action,  deleteitem,  functions }) {
  const DropDown1 = [
    {
      title: 'Edit',
      icon: FiEdit,
      ...
      <thead className="bg-dry rounded-md overflow-hidden">
        <tr>
          ...
          <th className={thclass}>
            Amout <span className="text-xs font-light">(USD)</span>
          </th>
        ...
        {data?.map((item, index) => (
          ...
          >
            ...
            <td className={tdclass}>
              <div className="flex gap-4 items-center" style={{justifyContent: 'left'}}>
                <span className="w-12">
                  <img
                    src={item?.patientAvater?item?.patientAvater:'http://placehold.it/300x300'}
            ...
            <td className={tdclass}>{item?.Day} {item?.Month} {item?.Year}</td>
            <td className={tdclass}>
              <span
                className={`py-1 px-4 ${
                  item.status === 'Paid'
...
// patient table
export function PatientTable({ data, functions, deleteitem, used }) {
          ...
          >
            ...
            <td className={tdclasse}>
              <div  className="flex gap-4 items-center" style={{justifyContent: 'left'}}>
                ...
                <div>
                  <h4 className="text-sm font-medium">{item.Surename}</h4>
                  <p className="text-xs mt-1 text-textGray">{item.Phone}</p>
                </div>
              </div>
            </td>
            <td className={tdclasse}>{item.Month} {item.Day} {item.Year}</td>
            ...
            {!used && (
              <>
                ...
                <td className={tdclasse}>{item.dob}</td>
              </>
            )}
...
// doctor table
export function DoctorsTable({ data, functions,  deleteitem, doctor }) {
  const DropDown1 = [
    {
      title: 'View',
      icon: FiEye,
      ...
      <tbody>
        {data?.map((item, index) => (
          ...
          >
            ...
            <td className={tdclass}>{item.Month}, {item.Day} {item.Year}</td>
            <td className={tdclass}>
              <p className="text-textGray">{item.Phone}</p>
            </td>
            <td className={tdclass}>{item.Surename}</td>
          ...
          >
            ...
            <td className={tdclass}>
              <h4 className="text-xs font-medium">
                {item?.doctorName?item?.doctorName:"Not set"}
              </h4>
              <p className="text-xs mt-1 text-textGray">
                {item.patientPhone}
              </p>
    ...
    <table className="table-auto w-full">
      ...
      <tbody>
        {data?.map((item) => (
          ...
          >
            <td className={tdclass}>
              <p className="text-xs">{item.Day}-{item.Month}-{item.Year}</p>
            </td>
            <td className={tdclass}>
              <h4 className="text-xs font-medium">
                {item.patientName}
