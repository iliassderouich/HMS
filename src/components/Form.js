...
export function Input({ label, name, type, color, placeholder, register, value, onChange }) {
...
export function Button({ label, onClick, loading, Icon }) {
  return (
    <button
      disabled={loading}
      onClick={onClick}
      className={`w-full flex-rows gap-4 hover:opacity-80 transitions bg-hkj-subMain text-white text-sm font-medium px-2 py-4 rounded`}
    >
      {loading ? (
