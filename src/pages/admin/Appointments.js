  ...
  return (
    <div className="flex flex-col gap-8 mb-8">
      ...
      <div className="grid sm:grid-cols-2 md:grid-cols-12 gap-4">
        ...
        {/* filter */}
        <div className="md:col-span-2 grid grid-cols-3 rounded-md  border-hkj-subMain">
          {viewNamesGroup.map((item, index) => (
            <button
              ...
              className={`text-xl py-2 flex-colo border-hkj-subMain ${
                toolbar.view === item.view
                  ? 'bg-hkj-subMain text-white'
                  : 'text-hkj-subMain'
              }`}
            >
              {item.view === 'month' ? (
...
function Appointments() {
  ...
  const events =   allAppointmentData.map(event => ({
    ...event,
  ...
  const doctorData = alluserData?.filter(member => member.Role === "doctor").sort((a, b) => b.ms - a.ms);
